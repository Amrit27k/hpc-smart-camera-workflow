...
6. Prints a summary table and writes:
      mc_results/
          ...
          mc_summary.csv               ← one row per edge×timestep
...
def parse_edgedata(path: Path) -> dict:
    ...
    for interval in tree.getroot().findall("interval"):
        ...
        for edge in interval.findall("edge"):
            ...
            data[(begin, end, eid)] = {"speed": speed, "density": density}
    return data
...
def write_csv(aggregated: dict, path: Path) -> None:
    with open(path, "w", newline="", encoding="utf-8") as fh:
        ...
        writer.writerow([
            "interval_begin_s", "interval_end_s", "interval_begin_hhmm",
            "edge_id",
            "speed_mean_ms", "speed_min_ms", "speed_max_ms", "speed_std_ms",
            "density_mean_vkl", "density_min_vkl", "density_max_vkl", "density_std_vkl",
            "n_runs",
        ])
        for (begin, end, eid), v in sorted(aggregated.items()):
            writer.writerow([
                begin, end, seconds_to_hhmm(begin),
                eid,
                f"{v['speed_mean']:.4f}",    f"{v['speed_min']:.4f}",
                f"{v['speed_max']:.4f}",    f"{v['speed_std']:.4f}",
                f"{v['density_mean']:.4f}",  f"{v['density_min']:.4f}",
                f"{v['density_max']:.4f}",  f"{v['density_std']:.4f}",
                v["n_runs"],
            ])
...
def main() -> int:
    ...
    # Write outputs
    write_csv(aggregated, out_root / "mc_summary.csv")
    write_report(aggregated, report_edges,
    ...
    log.info("  mc_summary.csv    — full edge × timestep table")
    log.info("  mc_report.txt     — human-readable prediction")
